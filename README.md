...
- **`math-literature` 更新**：用于文献与权威来源检索、元数据与 DOI/BibTeX 核验，以及 claim-to-citation 映射。它严格区分“为论文事实找证据”和“为建模找思路”：后者会转交 `math-research`，避免事后为了既定模型反向拼接引用。
- **`math-writing` 更新**：负责把已验证的模型证据改写成论文正文、结果分析和模型评价，阻止 raw output、诊断日志和无证据模板句进入正文。
...
| `math-writing` | 正文写作、段落修复、结果分析和 raw output 护栏 |
...
用 math-writing 把已验证结果改写成论文正文和结果分析。
