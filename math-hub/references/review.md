...
- 图表是否可读
- 排版是否影响可信度
- 图表、公式或表格是否超出正文宽度，或底部浮动体是否割裂上下文
- 结论是否有证据链
...
- 若建模路线来自文献启发，`method_source_matrix.csv` 是否记录任务匹配、数据匹配、验证负担和引用边界；是否已由 `math-model` 正式选择、由 `math-literature` 核验可引用来源。
...
- 检查图表是否在页面内完整显示；底部图片若把引入文字和图后结论分开，应调整浮动体位置或允许上方留少量空白以保持阅读连续。
